...
- 支持三个动作：
- `submit_topic_decisions`：选中的记录写为 `生成脚本包`，未选中写为 `不做`；选中记录同时写入 `制作方向卡状态=待发送`、`选择提交批次`、`选择提交时间`，作为第二张卡的显式发送队列。旧状态 `进入Brief` 仍作为队列兼容值。
...
- 第一张选题卡只允许处理一次。receiver 写入前会读取候选记录当前 `状态`，只有 `待判断` 或空状态可被处理；如果旧卡再次提交，已变成 `生成脚本包`、`进入Brief` 或 `不做` 的记录会触发拦截。
