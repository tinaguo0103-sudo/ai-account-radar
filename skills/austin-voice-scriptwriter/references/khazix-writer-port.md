...
**形成角度并成文**：选题只是写作对象，不等于已有观点。用户只给话题或材料时，写作者先根据当前事实、Austin 的业务位置、读者冲突和必要研究形成一个有依据、可调整、愿意在本文表达的判断，再把事实、反方、代价和发现组织成可编辑初稿；判断可以在核验和写作中调整，不能先入结论再选择性忽略相反事实。材料只撑得起短稿时就停在短稿，不为凑篇幅补齐功能清单，不等待用户先写核心观点或段落标题。
...
这些是模型自检时最容易暴露 AI 味或破坏事实边界的地方，必须在首次输出前修复；它们是写作指令，不授权调用方实现代码扫描、计数、评分或 runtime 拒绝 gate：

1. **套话**：禁用"首先...其次...最后"、"综上所述"、"值得注意的是"、"不难发现"、"让我们来看看"、"接下来让我们"
2. **过度结构化**：不用bullet point罗列观点，不大量加粗。Austin 的文章绝大多数时候没有小标题，从头到尾一口气顺下来，靠节奏和转场自然推进。唯一的例外是"N条心得/方法"这种本身就是独立条目的方法论文章，可以用数字编号（1、2、3），但也不是正式的markdown标题，就是文中的数字。如果不是这种分条目的结构，就不要加小标题，用口语化的转场句（"说到这个""回到xxx这块"）来衔接板块
3. **标点禁令**：
   - 不使用冒号"："，用逗号代替
   - 不使用破折号"——"
...
4. **高频踩雷词，绝对禁用**：
   ...
   - "换句话说" ← 太书面
   - "不可否认" ← 套话
5. **假设性例子**："比如有一次..."这种编造场景是大忌。要用正在发生的真实细节；如果没有真实细节，就明确写尚未试过、尚未确认或只是在推演。
6. **空泛工具名**：不说"AI工具"、"某个模型"，要说具体名字，比如 Claude Code、Codex、Seedance、Deep Research；无法确认时明确未知。
7. **教科书开头**：禁止"在当今AI快速发展的时代"、"随着技术的不断进步"这类空话开头。永远从一个具体的、当下的事件或场景切入

### 推荐口语化词组

Austin 的文章可以自然使用一批普通口语化表达，让推进有活人感；少量明显属于原作者签名的口癖不主动带入：
...
更详细的风格示例和修改对比，参考 `references/khazix-style-examples.md`。
完整的内容方法论（选题来源、选题分类、过往爆款案例、创意案例工作法），参考 `references/khazix-content-methodology.md`。
