...
当前生产路径已经改为 Codex App automation + Codex 定时 runner。n8n 只作为历史替代方案或以后跨系统编排参考，不作为当前无人值守生产路径。
...
4. Execute Command：调用 `scripts/codex_script_package_runner.py --write-feishu --limit 2 --max-age-days 5` 生成本地 `full_script_execution_package.md`，仅作为替代编排方案。
...
注意：如果要拆拍摄、录屏、剪辑、发布任务，需要后续单独设计任务表，不恢复 `05 Brief与制作`。
