...
最新主编规则是“三阶段主编工作流”：Skill 不能先套标题，也不能靠代码里的标题模板兜底。第一阶段只做 `主编筛选` 和 `主编自由稿`，判断这条到底值不值得我做；第二阶段做 `标题工作坊`，按痛点直击版、真实踩坑版、热点借势版、项目现场版、结果反差版、口播开场版至少发散 6 个不同标题方向；第三阶段做 `标题自审`，选出最像我、最有点击理由、最不模板的一条，再写 `点击钩子`、`观众为什么会点` 和飞书前台字段：`我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题`。只有筛选、标题发散、自审和提案卡都能写清楚，才允许生成可发布标题；否则最多进入本地暂存观察，不写入飞书前台候选池。
...
- `scripts/editorial_skill_runner.py`：全局 Skill 的主编层执行脚本，默认调用本机已登录的 Codex CLI，读取 `ai-account-editorial-director` 与案例库后重判候选。它要求 Skill 先完成 `主编筛选 / 主编自由稿`，再做 `标题工作坊 / 标题自审`，最后抽取 `点击钩子 / 观众为什么会点 / 我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题` 这张主编提案卡，并补充 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 等业务字段。`--engine deterministic` 只作为显式离线应急选项。
