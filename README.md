...
- `scripts/editorial_skill_runner.py`：全局 Skill 的主编层桥接脚本，按 `ai-account-editorial-director` 的字段契约补齐 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 等业务字段。
...
`daily_pipeline.py` 会串起：读取内容源配置、抓取 AIHOT、读取手动公众号/视频样例、生成 ContentItem、生成内容拆解、代码初筛今日候选池、通过 `editorial_skill_runner.py` 套用 `ai-account-editorial-director` 主编层字段、dry-run 展示、按需写入飞书、按需刷新主控台和输出日志。默认不写入飞书，不自动发布，不生成完整成稿。每次运行会生成一个 `运行批次`，用于在 `03 内容收件箱 / 今日采集` 和 `04 分析与选题 / 今日候选池` 中追踪本轮数据。
