...
最新主编规则是“真实矛盾 + 主编提案卡前置”：Skill 不能先套标题，必须先写出 `我的真实矛盾`，也就是这条内容和用户真实业务现场之间的冲突是什么。然后再判断旧流程原来怎么做、谁参与、卡在哪里，AI 具体改哪一步，用户保留什么判断，最后能展示什么证据。最终压成飞书前台可读的 `我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据`。只有这张提案卡能写清楚，才允许生成可发布标题；否则最多进入本地暂存观察，不写入飞书前台候选池。
...
- `scripts/editorial_skill_runner.py`：全局 Skill 的主编层执行脚本，默认调用本机已登录的 Codex CLI，读取 `ai-account-editorial-director` 与案例库后重判候选，并优先输出 `我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据` 这张主编提案卡，再补充 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 等业务字段。`--engine deterministic` 只作为显式离线应急选项。
