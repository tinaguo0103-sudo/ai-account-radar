...
最新主对标池以这版为准，旧对标账号池已降级为历史参考/备用观察池，默认不参与今日 Top10 主流程。`01 来源与采样` 会显性展示 `来源角色`、`是否主对标`、`是否参与主采样`、`栏目`、`栏目权重`、`默认启用` 和 `优先级`。
...
- `manual_entry`：手动入口，例如手动链接/粘贴/截图转文字、粘贴入口、`02 URL投喂入口`。
...
`04 分析与选题` 已收敛为今日 Top10 决策表：主字段 `选题标题` 就是最终给你看的选题标题，不再保留重复的 `我的选题标题` 字段。
...
当前除主控台和任务表外，每张飞书表只保留一个主视图；`00 主控台` 有两个入口视图：
...
其中 `02 URL投喂入口` 是输入层，不是任务表；`03 内容收件箱` 是所有采集内容的后台池；`04 分析与选题 / 今日Top10` 是选题决策区；`06 内容任务主表` 才是写稿、拍摄、剪辑、封面、发布、直播、复盘、私信跟进和资产化任务的执行表。
