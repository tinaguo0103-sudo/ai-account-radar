...
最新主编规则是“两阶段主编法 + 点击钩子层”：Skill 不能先套标题，也不能靠代码里的标题模板兜底。它必须先写一段 `主编自由稿`，像用户自己在选题前写给自己的判断备忘；再写 `点击钩子` 和 `观众为什么会点`，说明标题靠什么真实痛点、反常识、踩坑、结果差距或业务现场把人拉进来；最后从自由判断和点击理由里提炼飞书前台字段：`我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题`。只有自由稿、点击理由和提案卡都能写清楚，才允许生成可发布标题；否则最多进入本地暂存观察，不写入飞书前台候选池。
...
- `scripts/editorial_skill_runner.py`：全局 Skill 的主编层执行脚本，默认调用本机已登录的 Codex CLI，读取 `ai-account-editorial-director` 与案例库后重判候选。它先要求 Skill 写 `主编自由稿 / 点击钩子 / 观众为什么会点`，再抽取 `我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题` 这张主编提案卡，并补充 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 等业务字段。`--engine deterministic` 只作为显式离线应急选项。
...
`04 分析与选题` 已收敛为今日候选池决策表：主字段 `选题标题` 优先展示可读标题，用于日常决策和进入 Brief。飞书默认视图优先展示主编提案卡字段，例如 `今日建议级别`、`编辑判断分`、`AI味风险`、`内容可信度`、`推荐动作`、`主编自由稿`、`点击钩子`、`观众为什么会点`、`我的真实矛盾`、`选题判断`、`原始钩子`、`我的切入`、`我准备怎么讲`、`可展示证据`、`原始来源标题`、`推荐理由`、`不建议做的原因` 和 `可沉淀资产`。`一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 等推理补充字段保留在详细视图和本地调试文件里，但不作为默认前台阅读重点。写入前会先经过 `content_sampler.py` 的初筛和 `editorial_skill_runner.py` 的真实 Skill 主编判断，让你能区分“今日最值得做”“可选候选”“暂存观察”和“不建议制作”。没有足够人设角度或内容支撑的内容不会为了凑数进入候选池。

`暂存观察` 和 `不建议制作` 只保留在本地全量输出与调试文件里，默认不写入飞书 `04` 前台候选池。飞书里的 `04` 应代表“值得用户打开看的候选”，不是所有被系统扫到的素材。

飞书字段显示原则：代码和本地 CSV 可以保留完整调试字段，但飞书默认视图只露出业务判断需要的字段。`03 内容收件箱 / 今日采集` 默认看标题、来源、链接、摘要、正文长度、是否全文解析、解析说明、采集/处理状态和最近采样时间；内容指纹、payload 路径、运行批次等排障字段默认隐藏。`04 分析与选题 / 今日候选池` 默认看标题、建议级别、编辑判断分、AI味风险、可信度、推荐动作、来源、栏目、`主编自由稿`、`我的真实矛盾`、`选题判断`、`原始钩子`、`我的切入`、`我准备怎么讲`、`可展示证据`、推荐理由、不建议做的原因和可沉淀资产；`一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 保留在详细视图和本地调试文件里，作为提案卡的补充，不再作为默认前台阅读重点。
