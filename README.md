...
最新主编规则是“两阶段主编法”：Skill 不能先套标题，也不能靠代码里的标题模板兜底。它必须先写一段 `主编自由稿`，像用户自己在选题前写给自己的判断备忘，说明来源触发了什么、它和哪个真实/相邻业务现场有关、旧流程卡在哪里、准备拿它改/测/验证什么、不能声称什么、到底值不值得做。然后再从这段自由判断里提炼飞书前台字段：`我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题`。只有自由稿和提案卡都能写清楚，才允许生成可发布标题；否则最多进入本地暂存观察，不写入飞书前台候选池。
...
- `scripts/editorial_skill_runner.py`：全局 Skill 的主编层执行脚本，默认调用本机已登录的 Codex CLI，读取 `ai-account-editorial-director` 与案例库后重判候选。它先要求 Skill 写 `主编自由稿`，再从自由稿抽取 `我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题` 这张主编提案卡，并补充 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 可调用案例 / 证据强度` 等业务字段。`--engine deterministic` 只作为显式离线应急选项。
