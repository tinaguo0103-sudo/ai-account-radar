...
最新主编规则是“三阶段主编工作流”：Skill 不能先套标题，也不能靠代码里的标题模板兜底。第一阶段只做 `主编筛选` 和 `主编自由稿`，判断这条到底值不值得我做；第二阶段做 `标题工作坊`，至少发散多个不同标题方向；第三阶段做 `标题自审`，选出最像我、最有点击理由、最不模板的一条，再写 `点击钩子`、`观众为什么会点` 和飞书前台字段：`我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 可发布标题`。只有筛选、标题发散、自审和提案卡都能写清楚，才允许生成可发布标题；否则最多进入本地暂存观察，不写入飞书前台候选池。
