...
最新主编规则是“主编提案卡前置”：Skill 不能先套标题，必须先判断旧流程原来怎么做、谁参与、卡在哪里，AI 具体改哪一步，用户保留什么判断，最后能展示什么证据。然后压成飞书前台可读的 `选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据`。只有这张提案卡能写清楚，才允许生成可发布标题；否则最多进入本地暂存观察，不写入飞书前台候选池。
