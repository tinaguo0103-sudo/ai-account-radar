...
- `今日决策看板`：按 `状态` 看候选从 `待判断` 到 `生成脚本包`、暂存或不做的推进；不再兼容旧状态。
...
4. 进入 `04 分析与选题` 做选题决策：日常只需要确认哪些候选进入 `生成脚本包`，剩下的暂存、归档或不做；旧状态不再触发脚本包生成；尽量补一个 `选择原因标签`，方便后续学习你的判断逻辑。
...
1. 打开 `00 主控台 / 今日工作台`。
2. 先看 `04 / 今日挑选卡片`、`待生成脚本包`、`已生成脚本包`、`待处理与异常`。
...
6. 本机轻量 watcher 会把近 5 天已确认推进且未生成脚本稿的选题写入 `06 完整脚本与制作包`，完整 Markdown 落到项目根目录 `06 完整脚本与制作包/`；没有待生成记录时只做队列检查，不消耗 Codex。
...
说明：脚本会确保核心视图存在。`06 完整脚本与制作包` 只保留 `脚本包主视图`、`待处理与异常` 两个业务视图；完整内容看飞书文档或本地 Markdown，飞书表格只放业务摘要、提醒和同步状态。
...
3. 如果不用交互卡片，再进入 `04 分析与选题 / 今日挑选卡片`，手动决定生成脚本包、暂存、归档或不做，并尽量补 `选择原因标签`；只有 `生成脚本包` 会触发脚本包生成。
...
- `立即蹭热点`：每天 3-4 条，适合短视频、小红书短帖、公众号短评，不一定需要完整脚本包。
- `生成脚本包`：每天最多 1-2 条，必须适合沉淀成流程、清单、案例、长文或资料包。
- `暂存观察`：重要但需要补材料、角度还不够明确或不适合今天推进。
- `不做`：和定位无关，或只能资讯搬运。
