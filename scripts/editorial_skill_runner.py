...
def enrich(row: dict[str, str]) -> dict[str, str]:
    ...
    out["标题工作坊"] = row.get("标题工作坊") or row.get("可发布标题") or row.get("我的选题标题", "")
    out["标题自审"] = row.get("标题自审") or "保留热点入口，落到我的业务现场；如果只像正确摘要，就不要进入前台强候选。"
...
- 这一版必须执行“三阶段主编工作流”，不能一次性填表：
  1. `主编筛选`：只判断这条值不值得我做。先写清它是不是资讯搬运、能不能接到我的真实/相邻业务现场、证据够不够、应该做/存/补证据/放弃。这个阶段不要写标题。
  2. `标题工作坊`：只处理表达。至少发散 6 个不同标题方向，必须逐项写出：痛点直击版、真实踩坑版、热点借势版、项目现场版、结果反差版、口播开场版。它们必须真不同，不允许只是同一句式换词。每个标题方向都要短、能口播、有停顿点，不要解释成长段。
  3. `标题自审`：从标题工作坊里选 1 个最像我、最有点击理由、最不模板的标题；说明淘汰了哪些方向，为什么。自审必须额外回答“一个不了解我项目的陌生内容从业者，刷到这句话会不会停一下？”如果不会停，宁可重写或降级，也不要选一句安全正确的标题。然后才写 `可发布标题`、`点击钩子`、`观众为什么会点` 和其他飞书字段。
- `主编自由稿` 是阶段 1 的自然判断稿，长度约 120-220 字，像用户自己在选题前写给自己的判断备忘。它要自然写清：来源触发了什么、它和我的哪个真实/相邻业务现场有关、旧流程卡在哪里、我准备拿它改/测/验证什么、我不能声称什么、它到底值不值得做。
- `点击钩子` 和 `观众为什么会点` 必须来自阶段 2 和阶段 3，不是给钩子分类，而是说明这条标题用什么真实痛点、反常识、结果差距、踩坑、返工、交付冲突或业务现场把人拉进来。
- 最后从 `主编筛选 + 主编自由稿 + 标题工作坊 + 标题自审` 里提炼 `可发布标题`、`我的真实矛盾`、`选题判断`、`我的切入`、`我准备怎么讲`、`一句话Brief` 等结构化字段。不要先写字段再倒推自由稿。
- `主编自由稿` 不能写成字段清单，也不能像助理汇报；它应该像一个懂业务、懂内容、懂导演的人在临拍前想清楚“我为什么要讲这条”。
- `标题工作坊` 不能只是在同一个句式里换词。如果 5 个草稿看起来只是同一模板的变体，必须重写。
- `标题工作坊` 里至少要有 2 个标题比最终标题更大胆；如果全部都很稳、很正确、很像内部备忘，说明还没进入标题工作坊。
- `标题自审` 要明确回答：这句标题有没有让人想点？有没有具体热点或真实场景？是不是只是一句正确判断？像不像用户自己会说？陌生人刷到会不会停一下？
- `点击钩子` 必须能让目标观众产生“这说的是我遇到的问题/我可能踩过/这里有反常识”的感觉。只正确、不想点，就不合格。
...
- 标题还必须通过“点击理由测试”：如果标题只是正确的一句话，但没有冲突、痛点、结果差距、踩坑感或业务现场，就重写。
- 标题还必须通过“外部钩子测试”：即使不认识用户，也能看出这条和自己的内容生产、交付返工、团队协作、工具选型或业务判断有关。只对用户自己有意义的内部备忘，不适合作为可发布标题。
...
- 每行必须先输出 `主编筛选`、`主编自由稿`、`标题工作坊`、`标题自审`、`点击钩子`、`观众为什么会点`、`我的真实矛盾`、`选题判断`、`原始钩子`、`我的切入`、`我准备怎么讲`、`可展示证据`。这些字段是主编过程和前台提案卡，必须像写给用户看的判断，不要像算法推理。
