...
- 这一版必须执行“三阶段主编工作流”，不能一次性填表：
  ...
  2. `标题工作坊`：只处理表达。至少发散 5 个不同标题方向，必须包括热点入口型、真实现场型、结果差距型、个人判断型；可以有反常识，但不要批量抬杠。每个标题方向都要短，不要解释成长段。
  3. `标题自审`：从标题工作坊里选 1 个最像我、最有点击理由、最不模板的标题；说明淘汰了哪些方向，为什么。然后才写 `可发布标题`、`点击钩子`、`观众为什么会点` 和其他飞书字段。
...
- `标题自审` 要明确回答：这句标题有没有让人想点？有没有具体热点或真实场景？是不是只是一句正确判断？像不像用户自己会说？
