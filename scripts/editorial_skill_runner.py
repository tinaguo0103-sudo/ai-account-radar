...
- 这一版必须执行“两阶段主编法”：
  ...
  2. 再写 `点击钩子` 和 `观众为什么会点`：不是给钩子分类，而是说明这条标题用什么真实痛点、反常识、结果差距、踩坑、返工、交付冲突或业务现场把人拉进来。
  3. 最后从 `主编自由稿 + 点击钩子 + 观众为什么会点` 里提炼 `可发布标题`、`我的真实矛盾`、`选题判断`、`我的切入`、`我准备怎么讲`、`一句话Brief` 等结构化字段。不要先写字段再倒推自由稿。
...
- 每行必须先输出 `主编自由稿`、`点击钩子`、`观众为什么会点`、`我的真实矛盾`、`选题判断`、`原始钩子`、`我的切入`、`我准备怎么讲`、`可展示证据`。这些字段是飞书前台主编提案卡，必须像写给用户看的判断，不要像算法推理。
