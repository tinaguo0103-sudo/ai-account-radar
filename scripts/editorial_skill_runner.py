#!/usr/bin/env python3
"""Apply the ai-account-editorial-director Skill contract to topic candidates.

This is the bridge between deterministic sampling code and the global editorial
Skill. It does not call an LLM; instead it normalizes the current candidate CSV
into the business-readable fields defined by the global Skill so Feishu 04 shows
the user-facing editorial judgement layer.
"""
...
def write_report(path: Path, rows: list[dict[str, str]], input_path: Path, output_path: Path) -> None:
    counts: dict[str, int] = {}
    for row in rows:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply ai-account-editorial-director fields to a candidate CSV.")
    ...
    write_report(report_path, enriched, input_path, output_path)
    print(json.dumps({
        "ok": True,
        "rows": len(enriched),
        "input": str(input_path),
