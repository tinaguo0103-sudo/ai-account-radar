...
def reverse_evaluation_rows(selected: list[dict[str, Any]], candidates: list[dict[str, Any]], item_by_fp: dict[str, Any]) -> list[ReverseEvaluationRow]:
    ...
    for row in candidates:
        ...
        reason = row.get("不建议做的原因") or row.get("降级原因") or row.get("推荐动作原因") or "未给出明确未选原因"
        if potentially_better and reason in {"", "暂无明显不做理由。", "未给出明确未选原因"}:
            reason = "疑似漏选：高适配对标内容未入选，且未给出成立的不选理由。"
...
def collection_coverage_report(sources: list[dict[str, Any]], probe_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    artifacts = {
        name: int(by_name[name].get("resolved_items") or len(by_name[name].get("video_links") or []) or 0)
