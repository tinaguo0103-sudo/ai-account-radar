...
RULES: tuple[FailureRule, ...] = (
    ...
    FailureRule(
        ...
        likely_cause="采集源为空、内容都被主编层过滤、latest_write 非正式写入，或今日批次没有生成候选 CSV。",
        impact="10:00 不会发选题卡，避免把历史候选当成今天候选发送。",
        actions=(
            "先看 output/latest_write/content_sampler_log.json 的 today_candidates 和 run_id。",
    ...
    FailureRule(
        ...
        patterns=("today_daily_pipeline_log_not_ok", "latest_write_not_generated_today", "pipeline_and_latest_write_run_id_mismatch", "latest_write_is_not_write_feishu_mode"),
        conclusion="今日候选新鲜度守卫阻止发卡。",
        likely_cause="08:00 采集未成功、latest_write 不是今天生成、run_id 不一致，或最新候选不是正式写飞书模式。",
        impact="10:00 不会发卡，避免误发旧候选或 dry-run 候选。",
        actions=(
            "先检查 output/logs/daily_pipeline_今天.json 和 output/latest_write/content_sampler_log.json。",
