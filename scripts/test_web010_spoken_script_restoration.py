...
class SpokenScriptRestorationTests(unittest.TestCase):
    ...
    def test_release_contract_uses_one_owner_and_direct_skill_stages(self):
        ...
        self.assertIn("source-to-thesis bridge", protocol)
        self.assertIn("audible main conclusion early", protocol)
        self.assertIn("each paragraph advance one idea", protocol)
        self.assertIn("robot action footage is real and not generated by Seedance", protocol)
        self.assertIn("proposed test unless the source reports that test", protocol)
