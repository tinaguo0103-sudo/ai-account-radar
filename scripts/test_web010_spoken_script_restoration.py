...
class SpokenScriptRestorationTests(unittest.TestCase):
    ...
    def test_release_contract_uses_one_owner_and_direct_skill_stages(self):
        ...
        self.assertIn("relevant existing project materials", protocol)
        self.assertIn("verifiable first-party public research", protocol)
        self.assertIn("editable first draft", protocol)
        ...
        self.assertIn("make the article stand on its own without playing or introducing the source video", protocol)
        self.assertIn("do not inherit its sequence, wording, rhetorical structure or conclusion", protocol)
