...
def record(value: str, run_id: str = RUN_ID, suffix: str = "") -> dict:
    return {"record_id": f"rec-{value}{suffix}", "fields": {"内容指纹": value, "运行批次": run_id, "最近参与运行批次": run_id}}
...
class AR034FContentLedgerReconcileTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.existing = [record(row.fingerprint) for row in self.items[:136]]
    ...
    def run_reconcile(self, store: FakeLedger, *, write: bool, validator=lambda: None, **kwargs):
        return reconcile.reconcile_missing_records(
            ...
            revalidate_plan=validator, write=write, sleep=lambda _: None, **kwargs,
        )
    ...
    def test_first_reconcile_writes_only_26_then_full_162_and_87_pass(self) -> None:
        ...
        self.assertTrue(report["ok"])
        self.assertEqual((store.calls, report["created"], report["classification"]["existing_unique_count"]), (26, 26, 162))
    ...
    def test_missing_is_not_duplicate(self) -> None:
        state = reconcile.classify_records([row.fingerprint for row in self.items], self.existing, RUN_ID)
    ...
    def test_timeout_after_commit_is_resolved_without_second_write(self) -> None:
        ...
        self.assertEqual(report["created"], 25)
    ...
    def test_timeout_before_commit_retries_only_absent_fingerprint(self) -> None:
        ...
        self.assertEqual(report["created"], 26)
    ...
    def test_rate_limit_after_commit_is_resolved_by_readback(self) -> None:
        ...
        def rate_limited(fields):
            payload = original_create(fields)
            if store.calls == 1:
                raise RuntimeError("HTTP 429 rate limit; status unknown")
            return payload

        report = reconcile.reconcile_missing_records(
            self.items, RUN_ID, self.closure, read_records=store.read, create_record=rate_limited,
            revalidate_plan=lambda: None, write=True, sleep=lambda _: None,
        )
        self.assertTrue(report["ok"])
        self.assertEqual(store.calls, 26)
        self.assertEqual(report["already_committed"], 1)
    ...
    def test_one_item_hard_failure_stops_and_preserves_prior_success(self) -> None:
        ...
        self.assertEqual((store.calls, report["failed"], len(store.records)), (1, 1, 136))
    ...
    def test_concurrent_insertion_is_already_committed(self) -> None:
        ...
        def concurrent_read():
            ...
            reads += 1
            if reads == 2:
                store.records.append(record(self.items[136].fingerprint))
