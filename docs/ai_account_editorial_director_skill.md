...
最新规则不是“热点映射到母场景后生成标题”，也不是“先填字段再拼标题”。每条候选必须先写 `主编自由稿`，再从自由稿里抽取结构化字段。
...
旧字段 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现` 仍可保留，但它们必须从 `主编自由稿` 自然抽出来，不再是默认前台阅读重点。写不出自由稿、`我的真实矛盾` 和这张提案卡的内容，不允许包装成 `今日最值得做`。
...
→ Skill 从自由稿输出第一人称主编提案卡：主编自由稿 / 我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据
...
- 降低 AI 味和模板味；
- 输出能直接给人审查的选题字段；
- 先写主编自由稿，再抽字段，避免从一个标题模板变成五个标题模板。
...
不要把 Skill 的人设判断继续拆成大量硬编码标题模板；代码只做采集、去重、清洗和质量校验，标题与字段的生成应来自主编自由稿。
