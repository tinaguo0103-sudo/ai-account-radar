...
## 两阶段主编法：先自由判断和点击理由，再抽字段

最新规则不是“热点映射到母场景后生成标题”，也不是“先填字段再拼标题”。每条候选必须先写 `主编自由稿`，再写 `点击钩子` 和 `观众为什么会点`，最后从自由判断和点击理由里抽取结构化字段。
...
`点击钩子` 不是标题模板，而是点击理由。它要说明这条内容靠什么真实痛点、反常识、踩坑、结果差距、返工、交付风险或业务现场把人拉进来。`观众为什么会点` 要具体到目标观众的工作焦虑、效率损耗、内容生产卡点或业务判断困惑。
...
旧字段 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现` 仍可保留，但它们必须从 `主编自由稿` 和点击理由自然抽出来，不再是默认前台阅读重点。写不出自由稿、点击理由、`我的真实矛盾` 和这张提案卡的内容，不允许包装成 `今日最值得做`。
...
- 降低 AI 味和模板味；
- 输出能直接给人审查的选题字段；
- 先写主编自由稿和点击理由，再抽字段，避免从一个标题模板变成五个标题模板，也避免标题正确但没人想点。
...
不要把 Skill 的人设判断继续拆成大量硬编码标题模板；代码只做采集、去重、清洗和质量校验，标题与字段的生成应来自主编自由稿和点击理由。
