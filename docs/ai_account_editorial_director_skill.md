...
最新规则不是“热点映射到母场景后生成标题”，而是先写出 `我的真实矛盾`，再做业务现场复盘，压成主编提案卡，最后才长出标题。每条候选必须先回答：

0. 这条内容背后，用户自己的真实业务冲突是什么。它不能只是“某工具更新了”“这个属于AI导演工作流”，而要像用户自己复盘项目时会说的话，例如“我现在不是缺热点，而是缺一套能解释为什么选它的 Brief 判断”；

1. 它触发的是哪个真实或相邻业务现场；
2. 旧流程原来怎么做，谁参与，卡在哪里；
3. AI 介入后具体少掉、合并、重写或字段化了哪一步；
4. 用户作为懂营销、懂内容、懂导演、正在做 AI 业务系统的人，保留什么关键判断；
5. 最后能展示什么证据：流程图、字段表、Brief、分镜、封面、成片、返修清单、复盘数据或案例截图。

这些内容会压缩成飞书前台最重要的六个字段：
...
旧字段 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现` 仍可保留，但它们是提案卡的补充，不再是默认前台阅读重点。写不出 `我的真实矛盾` 和这张提案卡的内容，不允许包装成 `今日最值得做`。
...
→ Skill 必须先写出“我的真实矛盾”，再完成业务现场复盘：旧流程、卡点、AI改造、人保留判断、可展示证据
→ Skill 输出第一人称主编提案卡：我的真实矛盾 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据
