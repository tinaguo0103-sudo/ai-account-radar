...
最新规则不是“热点映射到母场景后生成标题”，而是先做业务现场复盘，再压成主编提案卡，最后才长出标题。每条候选必须先回答：
...
旧字段 `一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现` 仍可保留，但它们是提案卡的补充，不再是默认前台阅读重点。写不出这张提案卡的内容，不允许包装成 `今日最值得做`。
...
→ Skill 必须先完成业务现场复盘：旧流程、卡点、AI改造、人保留判断、可展示证据
→ Skill 输出主编提案卡：选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据
