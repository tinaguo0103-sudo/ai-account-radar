...
它不是采集脚本，也不是自动成稿器，也不是标题模板器。它负责“主编判断”：这条内容能不能被我讲成自己的选题命题、应该接到哪个生产现场、旧流程痛点和 AI 介入点是什么、我能补什么经验、重点体现什么、是否值得进入 Brief。
...
`选题命题` 是 `04 分析与选题` 的核心前台字段。它不是来源标题，也不是可发布标题，而是“我准备把这条外部素材改造成什么业务现场命题”。`标题工作坊 / 标题自审` 可以作为本地调试字段保留，但不再是主流程必填字段。`title_permission` 是标题门控字段：只有 `可发布标题` 才允许写入 `可发布标题 / 标题备选`；`内部测试标题` 和 `不生成标题` 必须清空前台标题，但仍可保留内部 `选题命题`。代码仍可保留更多中间字段，但飞书前台应该优先展示这些业务字段。
...
最新规则不是“热点映射到母场景后生成标题”，也不是“先填字段再拼标题”。每条候选必须先做门控，再生成命题卡，最后才做标题包装：

1. `Gate / 主编门控`：只判断这条值不值得我做、证据够不够、是否允许生成标题。输出 `主编筛选 / 主编自由稿 / 我的真实矛盾 / 场景依据 / 证据强度 / title_permission`。
2. `Topic Proposition Card / 选题命题卡`：通过门控后，整理成用户能看的工作备忘。核心输出 `选题命题`，并补充 `选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据 / 一句话Brief / 我的场景拆解 / 我的思考点 / 重点体现 / 需要补的证据`。
...
→ Skill 从自由稿和点击理由输出第一人称选题命题卡：主编自由稿 / 点击钩子 / 观众为什么会点 / 我的真实矛盾 / 选题命题 / 选题判断 / 原始钩子 / 我的切入 / 我准备怎么讲 / 可展示证据
...
- 降低 AI 味和模板味；
- 输出能直接给人审查的选题字段；
- 先做 Gate 主编门控，再做 Pitch 提案卡，最后按 `title_permission` 决定是否生成标题，避免从一个标题模板变成五个标题模板，也避免标题正确但没人想点。
