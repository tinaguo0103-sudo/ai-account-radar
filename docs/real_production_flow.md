...
- `06 完整脚本与制作包`：保存脚本包记录、摘要和文档入口；完整正文优先看 `飞书文档` 链接，本地平铺 Markdown 是备份和后续自动化输入，不塞进飞书长字段。
...
- 完整 Markdown 从项目文档库根目录的 `06 完整脚本与制作包/YYYY-MM-DD_选题标题_完整脚本与制作包.md` 打开；后台真实写入 runtime 并通过软链接暴露到项目根目录。若 `FEISHU_SCRIPT_PACKAGE_FOLDER_TOKEN` 可用，runner 会同步创建飞书文档并把链接写入 `06 / 飞书文档`；飞书 `06` 只保存摘要、文档入口、素材提醒、发布前核验和 QA。
...
- 飞书文档生成到应用可写的 `06 完整脚本与制作包` 文件夹，并在飞书 `06 完整脚本与制作包` 新增一条轻量记录。
