...
- 已自动化：第一张选题卡由腾讯云 SCF receiver 接收并写回 `04`，选中记录会写入 `制作方向卡状态=待发送`；腾讯云定时触发器每 5 分钟扫描这个显式队列并发送第二张制作方向补充卡；第二张卡提交后继续由 receiver 写回 `04 / 我的制作补充`。本机轻量 watcher 扫描近 5 天待生成记录，空队列不调用 Codex，有待生成记录时才调用 Codex 生成 `06`。
