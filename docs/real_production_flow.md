...
- 只处理人工状态为 `生成脚本包` / `进入Brief` / `本周做`，且 `是否已生成脚本稿 != 是` 的记录；不再仅凭 AI 的 `推荐动作` 自动生成。
...
- 飞书文档优先生成到用户可见的 `06 完整脚本与制作包` 文件夹，并在飞书 `06 完整脚本与制作包` 新增一条轻量记录；如果当前 token 无权写入用户可见文件夹，06 会保留本地文档并写出同步报警。
