...
打开 `00 主控台 / 今日工作台` → 看交互式选题速选卡或 `04 / 今日挑选卡片` → 勾选值得推进的候选并补制作方向 → 运行 `learn_from_topic_selection.py` 生成待确认选择学习摘要 → 本机轻量 watcher 按需调用 Codex 写入 `06 完整脚本与制作包` → 打开 `06` 的本地文档路径看完整脚本、素材待办、剪辑交接和 QA → 发布后进 `07` 复盘。
