...
- `05 Brief与制作`：历史测试表，正式链路不再写入，也不再作为日常入口。
- `06 完整脚本与制作包`：保存已生成完整口播稿与制作执行包的轻量记录，包括状态、核心观点、开头钩子、本地文档路径、素材提醒、发布前核验和 QA；完整内容不塞进飞书。
- `07 资产与复盘`：发布后看数据、复刻价值、改角度再发和资产化机会。
- `99 规则与字典`：系统说明书，解释字段、状态、评分、AI 边界和表逻辑。

## 每天怎么用

打开 `00 主控台 / 今日工作台` → 看交互式选题速选卡或 `04 / 今日挑选卡片` → 勾选值得推进的候选并补制作方向 → 运行 `learn_from_topic_selection.py` 生成待确认选择学习摘要 → 运行 `content_ops_pipeline.py --write-feishu` 生成本地 `full_script_execution_package.md` 并写入 `06 完整脚本与制作包` → 打开本地执行包看口播全文、素材待办、剪辑交接和 QA → 发布后进 `07` 复盘。
