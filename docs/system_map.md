...
- `06 完整脚本与制作包`：保存已生成完整口播稿与制作执行包的记录，包括状态、核心观点、开头钩子、文档入口、素材提醒、发布前核验和 QA；完整正文看本地平铺 Markdown 脚本包，后续优先同步到飞书文档。
- `07 资产与复盘`：发布后看数据、复刻价值、改角度再发和资产化机会。
- `99 规则与字典`：系统说明书，解释字段、状态、评分、AI 边界和表逻辑。

## 每天怎么用

打开 `00 主控台 / 今日工作台` → 看交互式选题速选卡或 `04 / 今日挑选卡片` → 勾选值得推进的候选并补制作方向 → 运行 `learn_from_topic_selection.py` 生成待确认选择学习摘要 → 本机轻量 watcher 按需调用 Codex 写入 `06 完整脚本与制作包` → 优先打开 `06 / 飞书文档` 看完整脚本、素材待办、剪辑交接和 QA，本地文档路径作为备份 → 发布后进 `07` 复盘。
