...
- `01 来源与采样`：来源池和采样权重，区分当前主对标、历史对标、系统热点源、官方源和手动入口。
...
`01 来源与采样`、`02 URL投喂入口`、`03 内容收件箱`、`99 规则与字典` 都不是日常任务入口。只有补链接、排查 URL 解析、查看原始内容或调整规则时再打开。当前 URL 投喂只正式支持公众号文章、抖音单条视频、RSS/Atom 和普通网页。
