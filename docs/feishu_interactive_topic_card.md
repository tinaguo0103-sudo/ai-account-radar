...
目标：保留 `04 分析与选题` 作为事实源，但不再让用户在多维表格卡片视图里逐条点开阅读。每天把最新一批候选压成一张飞书交互卡片，用户在卡片内一次完成选择，提交后回写 `04`。
...
日常生产路径是：本机或自动化只负责 `send` 一张卡片，用户点击后的 `card.action.trigger` 由云函数 receiver 接收并回写 `04`。云函数代码在 `cloud_functions/feishu-card-receiver/`。
...
- 进入 Brief：只勾选值得继续生成脚本大纲的编号。
- 推进原因标签：用结构化标签记录为什么选。
- 手工原因：标签覆盖不了时，写一句真实判断。
- 提交选择：已选进入 `Brief`，未选自动写为 `不做`。
...
用户在卡片里点击“提交选择”或“本批都不选”后，云函数 receiver 会把这些字段写回 `04 分析与选题`：
