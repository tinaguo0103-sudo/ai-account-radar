...
2026-06-26 回调耗时实测后，补充一个边界：第一张卡“回写选择 + 继续发送第二张制作方向卡”在腾讯云 SCF 上仍可能超过飞书客户端等待窗口，导致前端显示“提交错误”但后端实际已经回写。已经做了三项优化：卡片 value 携带候选快照、receiver 缓存 token/table_id、云端配置 `FEISHU_TOPIC_TABLE_ID`。但腾讯云 SCF 会等待未完成的后台 HTTP 任务，不能把“发送第二张卡”伪装成真正异步。
...
当前产品决策是不合并两张卡。若后续仍遇到提交错误，下一版优先把第二张制作方向卡交给真正的异步队列、独立云函数或定时扫描任务发送。
