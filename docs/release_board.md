...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Production Release Authorized / In Progress | hotfix main -> canonical login -> automation resume | 用户已授权；生产线程按 pause/backup/stop/gate/login/read-back/resume 硬顺序执行，任一步失败保持 PAUSED |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Release In Progress / Pre-08:00 Blocker | authorized release -> canonical login -> scheduled smoke | 只有 production Git gate 与 canonical logged_in 均通过才恢复 08:00/09:15/10:00；无 fallback |
