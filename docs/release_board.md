...
- AR-034G visible result：`Business Recovery Complete / Topic Card Sent / Automations Awaiting Manual Resume`，message_id=`om_x100b6aa90ea9d480de2d14483d9b0e9`。唯一剩余项是用户在UI中保持projectless、把cwd从`~`改为production repo后启用三任务；live official update当前无法表达该组合。若UI无cwd字段则保持PAUSED；不创建项目任务、不raw-edit TOML、不重复发送card。
