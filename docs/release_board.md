...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | QA Recheck Failed / Final Consolidated Rework | final dev rework -> QA -> hotfix release -> profile migration | Unicode CLI 已通过；剩余 blocker 是 malformed markers/state payload 会抛异常而非 typed fail，scope 用 code-only patch/manifest 管理 |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Final Rework / Pre-08:00 Blocker | isolated dev -> QA -> hotfix main | 当前 9333 仍为旧 RC profile；需补 DOM payload schema typed failure，再做一次完整 QA |
