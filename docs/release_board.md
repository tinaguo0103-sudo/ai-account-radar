...
- AR-034E RC4 QA Passed / Awaiting User Production Authorization：独立QA确认writer input/read-back 162、source projection 87、75合法其他来源任意穿插、prewrite与post-write mutation、real run和full regression全部通过；PM接受。计划SHA=`cdb32920...`，动作止于exact RC release、03 full ledger/read-back、signed watermark、11-row exact editorial、04/read-back、一次personal card；automation不在本次授权内且保持PAUSED。
- AR-034E RC4 Released / 03 Partial Write：production=`07940e8...`，release gate通过。唯一一次03 writer后planned=162、unique matched=136、missing=26、wrong_run=0；实际duplicate group=0，旧validator把missing同时标成duplicate。已停止watermark/editorial/04/card，无重采/provider request，三automation继续PAUSED。
- AR-034F / Development：目标是幂等只补03缺失26条并完整回读162+有序87投影；不得重跑全量writer、修改现有136或继续下游。固定dev线程只做证据诊断、reconcile入口、对抗测试和current-production-base窄RC；完成后PM evidence review再决定是否派一次independent QA。
- AR-034F RC1 PM Review Failed / Development Rework：RC1=`461bd660...`的create-only恢复会为已有URL/title旧记录新增26条重复内容，未关闭根因。旧RC保留但不派QA。RC2必须唯一匹配canonical-less legacy record并只PUT fingerprint，真正无记录才POST；同时修复生产writer在short-text existing record上漏写fingerprint。要求当前形状26 PUT/0 POST、总记录数不增加、final 162+87、second run 0 write。
- AR-034F RC2 / Independent QA：RC2=`10489aae...`、base=`07940e8...`、4 files、patch=`e9f1d98b...`；PM focused 24/24与scope/root review通过。QA须fresh验证legacy adoption、truly absent create、ambiguous PUT read-back、future writer root fix、162+87与second no-op，并执行一次production GET-only返回实际26 action split；仅在conflict/duplicate/wrong-run=0且writes=0时可进入PM Production Authorization。
- AR-034F RC2 QA Failed / Development：live check-only=`136 exact +26 conflict`，0 write；发布时间空值被RC2强制当成identity mismatch，且URL item仍受same-title other URL污染，属于matcher过严。RC2不得恢复生产。RC3只把empty published_at视为unknown optional，并让nonempty URL成为唯一candidate anchor；URL空则需唯一完整composite，title-only仍禁用。一个fresh RC、一次full QA，不扩scope。
- AR-034F RC3 / Independent QA In Progress：RC=`ad3d48e...`，exact base=`07940e8...`，aggregate 4 files，patch=`23505db7...`。PM evidence gate通过；QA须独立验证matcher mutation、RC2 recovery合同、完整回归及当前生产162/26 GET-only action split。禁止Feishu mutation和所有下游动作；若任何live conflict仍存在则Fail，不为放行继续放宽身份规则。
- AR-034F RC3 QA Failed / Product-Data Decision：live exact classification仍为162 planned、136 exact、26 conflict、0 PUT/POST/write；RC3不得恢复生产。代码控制与回归绿色不覆盖live readiness。当前lane转为一次bounded GET-only field-difference diagnosis，不是RC4开发；输出只含match booleans/counts/typed labels，不含原始业务内容。字段根因明确前不改matcher、不写03、不继续watermark/editorial/04/card，三任务保持PAUSED。
- AR-034G / Development：PM已终止AR-034F recovery matcher路线。新合同以同批次unique existing record为canonical owner：稳定URL只用于去重/owner选择，不覆盖owner，也不作为跨URL/title fallback。当前期望162 raw ->140 owners、26 aliases、22折叠、4额外owner、03业务写入0；下游从140 owner universe重新计算或映射候选。dev须fresh production-base单RC、零外部调用，不自行派QA；三automation保持PAUSED。
- AR-034G RC1 PM Review Failed / Development Rework：RC1=`0981500e...`的当前恢复形状、alias immutability和零writer均通过，但future normal path在无existing record时未先对raw planned同URL分组，两个新重复项会create两条03记录。该缺陷直接违背业务去重目标，不是安全扩scope。RC1保留历史不派QA；RC2从production base聚合完整修复，要求same-key new items只产生1 owner/1 create、其余alias，second run no-op，同时保持current 162->140与评分/主编字节不变。
- AR-034G RC2 / Independent QA In Progress：RC=`207060c...`、exact base=`07940e8...`、5 files、patch=`1d919d72...`。PM证据复核与exact反例通过：same-key 2/3 new items只产生1 owner/create，second run new=0；current fixture仍162->140/26 aliases/0 new/0 writes。QA须一次性复核L0、future dedup、production GET-only owner/read-back/source counts、140 owner candidate projection与全回归；禁止任何业务写入，三automation保持PAUSED。
- AR-034G RC2 QA Passed / Production Authorized：RC=`207060c...`独立QA通过；live GET-only为162 raw、140 owners、26 aliases、0 new/0 write，owner分布87 Douyin/18 WeChat/35 AIHOT，候选11条无alias escape。用户明确要求不再修改、直接发布；PM停止追加rework并派固定production线程执行exact fast-forward、dynamic gate及现有恢复路径。三automation在业务恢复完成前保持PAUSED；不新建RC、不指定model/thinking。
- AR-034G RC2 Released / Check-only Smoke Passed：production main/local/remote=`207060c...`、tree=`2d73bffd...`、clean；dynamic gate和released-code owner smoke通过。live shape=162 raw/140 owners/26 aliases/0 new/0 write，candidate=11；无Feishu 03/04、watermark、editorial/card/collection/provider或automation动作。当前状态为Hotfix Released / Recovery Still Blocked，三automation继续PAUSED。
- AR-034G Same-day Recovery Running：用户授权继续正常业务流。因今天采集已存在且owner read-back green，本次跳过重采和provider refresh；production线程从现有同日数据执行owner-aware candidate regeneration、03 no-duplicate closure、watermark、current-task editorial、04 exact read-back、一次personal card，并在全绿后status-only恢复三automation。禁止代码修改、新RC、旧数据补位、06/callback及重复发送。
- AR-034G Recovery Continuation：140-owner/03零写、11-row候选及6-row主编存活结果已完成；04 dry-run计划4条。唯一阻塞是watermark缺独立CLI。PM不接受为此新增hotfix，已在原授权内批准released commit函数一次性exact调用与read-back；随后按原流程一次04、一次personal card、status-only resume。三automation当前仍PAUSED。
- AR-034G After-commit Continuation：watermark正式commit与canonical/DB read-back均成功；同receipt再跑health被anti-replay拒绝是预期阶段语义。PM已确认不要求该pre-commit verifier在after-commit重放green，现继续04 exact write/read-back、card guard/一次personal send及automation status-only resume。
- AR-034G 04 Correction Running：Feishu 04 exact 4 records已创建且read-back无重复；card仅因4条`可沉淀资产`同为泛化内部过程描述被阻断。production获准沿现有Stage2/finalize correction路径更新这4个具体可见字段，禁止新增记录/代码/来源；validator green后一次card与status-only resume。
- AR-034G production tail：`04 Content Correction Passed / State Closure Running / Automations Paused`。同批次 04 已 4/4 green，watermark accepted；待 production 通过 released project log helper 将 exact run 的 truthful partial 状态写为 `downstream_usable=true + editorial_finalized/recovered_ok=true`，随后 card check-only 放行才允许一次个人发送，发送 green 后仅 status-only resume 三条 automation。该项不是代码发布 lane，不再创建 RC/QA。
- AR-034G card tail：`State Closure Passed / One Source-display Correction Running / Automations Paused`。唯一阻塞为一条Douyin source title与caption重复；按既有“平台未提供独立标题”展示合同做exact field normalization，不改validator。card send green后才status-only resume。
- AR-034G visible result：`Business Recovery Complete / Topic Card Sent / Automations Active but Entrypoint Repair Pending`，message_id=`om_x100b6aa90ea9d480de2d14483d9b0e9`。用户已手工启用三任务，但read-back仍为projectless + cwd `~`，相对`scripts/...`入口尚不可运行；需在UI设置production cwd，或在每条prompt开头显式切到production repo。live official update当前无法安全表达该组合；不创建项目任务、不raw-edit TOML、不重复发送card。
