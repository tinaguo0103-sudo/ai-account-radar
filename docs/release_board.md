...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | In Development / Blocks Scheduled Collection | dev -> QA -> hotfix release -> production profile migration | 当前 9333 绑定旧 RC profile 且 logged_out；必须固定 worktree-independent profile、身份校验、登录预检和 fail-closed 日志 |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | In Development / Pre-08:00 Blocker | isolated dev -> QA -> hotfix main | 9333 actual profile mismatch + logged_out 已只读确认；不得复用错误 profile 或将登录失败 optional 化 |
