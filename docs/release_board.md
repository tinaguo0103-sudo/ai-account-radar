...
| AR-026 | 飞书 01 全量对标账号采集覆盖 | P1 | QA Passed / Waiting Release Authorization Plan | dev/test -> RC -> production release | Round 2 QA 通过；生产 01 当前仍未写入隔离，发布时需按 release sync plan 将 8 个污染源切到 `quarantined_source` 并 read-back |
