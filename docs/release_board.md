...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | QA Failed / Consolidated Rework in Development | dev rework -> QA -> hotfix release -> production profile migration | 身份链设计通过主要对抗项，但中文路径下 DOM probe CLI 无输出；修复真实 CLI 合同前不得组 Hotfix RC |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | QA Failed / Pre-08:00 Blocker | isolated dev rework -> QA -> hotfix main | 当前 9333 actual profile mismatch + logged_out；另有中文路径 CLI 入口阻断，集中返修后必须重做真实 spawn 与隔离 Chrome QA |
