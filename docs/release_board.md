...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Follow-up Release Authorized / In Progress / Automations Paused | Git-only release -> session read-back -> status-only resume | 用户已授权 exact RC `178f047`；不停止Chrome、不迁移profile，任一 mismatch 保持PAUSED |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Follow-up Release In Progress / Pre-08:00 Blocker | production execution -> logged_in read-back -> resume | 仅Git/gate/session/status动作获授权；scheduled-day smoke仍独立待验 |
