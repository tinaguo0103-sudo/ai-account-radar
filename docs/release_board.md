...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Visibility RC QA Passed / Ready for Authorization / Automations Paused | Git-only release -> session read-back -> status-only resume | RC `178f047` 发布 QA 与 fresh 9333 logged_in 均通过；等待用户授权最小 follow-up 发布 |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Ready for Minimal Production Authorization / Pre-08:00 Blocker | authorized Git release -> logged_in read-back -> resume | 不停止 canonical PID、不复制 profile；任何 mismatch 回滚 follow-up code并保持PAUSED |
