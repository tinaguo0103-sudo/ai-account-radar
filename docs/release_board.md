...
| AR-026 | 飞书 01 全量对标账号采集覆盖 | P1 | RC Release QA Failed / Consolidated Rework | dev/test -> RC -> production release | RC 0b5a98e 实际 Node 仍接受 12/3 并截断；须三层 pre-side-effect hard fail 后重建 RC |
...
| AR-026 | 飞书 01 全量对标账号采集覆盖 | P1 | RC Failed / Development Rework | feature/next-production-flow | 当前 RC 禁止发布；修复正 cap 截断并从 production 178f047 重建 fresh RC 后重新完整 QA |
