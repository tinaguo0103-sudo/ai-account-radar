...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Dev Rework Passed / Ready for QA Recheck | QA recheck -> hotfix release -> production profile migration | `ffe93e4` 已修中文/空格/symlink CLI 与严格 JSON/exit 合同；等待独立真实 spawn 和隔离 Chrome recheck |
...
| AR-031 | 固定抖音 Chrome Profile 与登录态硬门 | P1 | Ready for QA Recheck / Pre-08:00 Blocker | isolated QA -> hotfix main | 当前 9333 仍为旧 RC profile；CLI 阻断已返修，QA 通过和 production profile migration 前不得恢复采集 |
