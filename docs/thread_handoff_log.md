...
- 状态建议：AR-009 标为 `Needs Rework`，QA Lane 为 `User Rejected / Rework Needed`；当前不应把用户验收失败计入 dev/test 三轮返修。返修前必须由 PM 先对齐需求，再派发覆盖对标来源/表达拆解、知识库浅显科普、用户风格金标和真实样例人工确认点的开发/测试任务。
