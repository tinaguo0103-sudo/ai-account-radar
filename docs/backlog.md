...
### AR-012 PM 派发前需求对齐和返修轮次定义修正

- 类型：项目治理 / PM 门禁
- 优先级：P1
- 状态：Ready
- 来源：AR-009 暴露 PM 未先和用户对齐需求就派发，且误把“三轮测试”理解成用户人工返修轮次，而不是 dev/test 线程之间的返修复测循环。
- 影响：PM 如果没有先复述并确认关键验收点，开发和测试即使认真执行也可能验证错方向；返修轮次定义不清会导致用户验收被错误纳入工程返修计数。
- 发布策略：不进入业务代码发布；作为 PM/QA 流程立即生效。
- 验证方式：PM 派发前必须检查任务卡是否覆盖用户所有明确要求；内容质量、用户可见输出、发布门禁或高歧义需求，必须先复述目标、边界和验收口径，必要时等待用户确认。三轮返修只统计开发完成后测试不通过、开发修复、测试复测的 dev/test 循环，不统计用户与 PM 的人工需求澄清或验收反馈。
- 关联分支/提交：
- 备注：开发任务卡和测试任务卡必须使用同一套验收口径。测试任务不能只验证开发自己改了什么，而要验证用户原始要求是否被满足。
