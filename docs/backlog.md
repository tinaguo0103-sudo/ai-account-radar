...
- AR-034G / Manual Projectless Entrypoint Repair：业务恢复与个人Topic Card已完成。用户已手工将三automation切为ACTIVE，但read-back仍为projectless + `cwds=["~"]`，prompt继续使用相对`python3 scripts/...`，故尚不能证明明日可运行。需在UI把工作目录设为`/Users/congcong/Desktop/AI/AI项目/AI账号工作流/ai_account_radar`；若无cwd字段，则在每条prompt首行显式`cd`到该目录。保持projectless，不绑定项目、不raw-edit TOML。
