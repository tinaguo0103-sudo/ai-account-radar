...
- AR-034G / Manual Projectless Cwd Resume：业务恢复与个人Topic Card已完成，唯一未完成项是三automation仍PAUSED且`cwds=["~"]`。当前已安装App的live official update要求`projectId`并拒绝`cwds`，无法安全表达projectless + production cwd；用户将通过UI手工保持“不在项目中工作”、设置cwd=`/Users/congcong/Desktop/AI/AI项目/AI账号工作流/ai_account_radar`后启用。若UI不提供cwd字段则继续PAUSED，不只切ACTIVE、不绑定项目、不raw-edit TOML。
