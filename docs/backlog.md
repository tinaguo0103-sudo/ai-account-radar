...
- AR-034G / Official Projectless Cwd Resume：三automation当前为projectless但`cwds=["~"]`，会找不到生产脚本。PM核对官方`automation_update` schema后，已回派production：cron update需完整传现有定义的`id/kind/name/prompt/rrule/status/model/reasoningEffort/executionEnvironment/cwds`，不传`projectId/target`；设置`status=ACTIVE`与production cwd，保持projectless、schedule/prompt/model不变。逐条official view/read-back，无catch-up；禁止raw TOML。
