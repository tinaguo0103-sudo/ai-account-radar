...
- 验证方式：用昨天两个实际选题做本地回归样例；生成改前/改后对比；检查开头是否先落到具体场景、是否引用对标表达或账号内真实经验、是否避免“我要做知识库”这类先讲概念的开场；必须提供用户可人工确认的真实样例路径、关键片段或可读报告；不写生产业务表、不创建生产文档。
- 关联分支/提交：`019f484 feat: make 06 scripts scene-first`
- 备注：优化方向包括：先搜索/拆解别人怎么讲同类题或对标账号怎么表达；把抽象主题拆回用户账号里的真实工作流问题；知识库类选题必须先抛具体场景，再引出知识库作为解决方案；口播稿要体现个人使用体验、体验场景和细节，不只复用稳定框架。2026-07-03 已派发开发线程 `019f1de3-f3f2-71d2-ae63-a74cd38f8474` 和测试线程 `019f269e-e26b-74d2-8ba1-a606edef1171`。开发线程已完成仓库 Skill 镜像和 deterministic 回归测试；测试线程独立 QA 通过，但 PM 未同步给用户真实样例内容，用户无法人工确认，因此状态回退为 `Waiting User Review`。发布前仍需处理全局私有 Skill 同步策略，并在真实 watcher 环境做最小 production smoke；未完成前不得声称生产已启用。
