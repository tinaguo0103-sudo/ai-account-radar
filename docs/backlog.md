...
- AR-035 / Script Package Watcher Codex CLI Path Repair：Development。第二张 Topic Card 的点击、receiver 与队列均正常，但 production watcher 仍硬编码已不存在的 `/Applications/Codex.app/Contents/Resources/codex`，当前安装路径为 `/Applications/ChatGPT.app/Contents/Resources/codex`，导致生成器尚未启动、本地和飞书 06 都为 0。固定开发线程仅修 runner/installer 共用的 CLI 解析并形成 production-base 窄 RC；当前不改 production/runtime/LaunchAgent/Feishu，不新增重试、身份或安全架构。PM 误建的 scratch worktree 保持未提交、不得作为发布候选。
