...
- AR-034E RC4：PM evidence review Passed / Production-shape QA Dispatched。fresh RC=`release/ar034e-rc4-20260717@07940e899e08201ee42528fbb42782ea5410acce`，direct parent=`d88d0e5...`，tree=`5ec34359...`，patch SHA=`b2549deb...`，manifest SHA=`e760c2b7...`。PM用RC4真实validator重放87 planned+75合法其他来源，结果full ledger=162、source projection=87、ordered equality=true；新增三项production-shape test通过，缺失/重复/错序/wrong-run/source-fingerprint/count/malformed均阻断。manual truth、writer前zero-call和forbidden scope保持。当前只进入一次独立QA，不生产、不调用Feishu/provider、不改automation。
- AR-034E RC4 QA/PM acceptance：Production-shape Independent Full QA Passed，PM接受进入用户生产授权。fresh clone、3-file hunk scope、public writer 162 input/read-back、87 ordered Douyin projection、prewrite zero-writer mutations、post-write projection mutations、真实同日run与400 Python均通过；生产0动作。授权计划=`/private/tmp/ar034e_rc4_production_authorization_20260717/PRODUCTION_AUTHORIZATION_PLAN.md`，SHA256=`cdb32920fa993d31d2c163c9dc6b7de453cb14075b2844c7d43496c1896b7c82`。计划仅允许Git release、复用现有162写03/read-back、existing signed watermark、11-row exact current-task、04/read-back与一次personal card；禁止重采集/第二refresh/read/06/automation change，三任务继续PAUSED。等待用户明确确认。
- AR-034E production：RC4已发布至production `07940e899e08201ee42528fbb42782ea5410acce`，动态gate通过且无重采/provider请求。唯一一次03 writer输入完整162，但read-back只有136个唯一fingerprint，missing=26、wrong_run=0；当前`duplicate=26`只是旧分类把count=0也计入duplicate，实际multi-record fingerprint=0。按硬门停止，未推进watermark、主编、04或卡片，三automation保持PAUSED。证据=`/private/tmp/ar034e_production_20260717_121830`。
- AR-034F Development：只修Feishu 03部分写入的幂等reconcile。固定dev线程需先查明26条共同特征与writer部分成功可见性，再交付exact-run/check-only/only-missing补写、timeout-after-commit read-back、二次执行no-op、完整162 read-back+87投影验证的production-base窄RC。开发/测试禁止调用Feishu或任何外部系统，不允许整批162重写、删除/更新现有136、继续watermark/editorial/04/card或派QA。
- AR-034F RC1 PM evidence review Failed / Consolidated Rework：RC1=`461bd660...`正确查明26条由旧URL/title identity命中但PUT未补`内容指纹`，却把它们当完全缺失并计划POST 26条新记录；这会保留26条canonical-less旧行并新增同内容行，造成业务重复。QA未派、RC1不得发布。固定dev线程须改为136 exact untouched + unique compatible legacy record仅PUT fingerprint + truly absent才POST，并修复未来short-text existing record的fingerprint update；生产形状应证明26 PUT/0 POST、record count不增加、final 162+87与second-run no-op。冲突/歧义旧记录必须写前阻断。
- AR-034F RC2 / Independent Full QA：fresh RC=`release/ar034f-rc2-final-20260717@10489aae783288d242305eada988e406c6c4d383`，direct parent=`07940e8...`，4 files，patch SHA=`e9f1d98b...`。PM证据复核确认future writer对empty fingerprint existing row独立于fulltext补canonical fingerprint，reconcile优先收养unique strong legacy并只在truly absent时POST；fixture为136 exact+26 legacy、26 PUT/0 POST、record count不增、second-run 0 write。固定QA线程已接收一次完整QA，并获准做一次生产Feishu GET-only分类以返回实际26条PUT/POST/冲突拆分；业务写入和其他外部动作仍为0。
- AR-034F RC2 QA Failed / RC3 matcher calibration：代码scope、fixture与424 Python通过，但生产GET-only为136 exact /0 legacy /0 absent /26 conflict。根因是RC2把planned empty `发布时间`也要求candidate非空精确相等，且有URL时仍把same-title/different-URL行加入potential conflict；这是过严可选字段规则，不是真实26条均歧义。固定dev线程仅校准：nonempty URL只按exact URL锚定；planned published_at为空时忽略该可选比较，非空时仍精确；URL空时禁止title-only并要求唯一title+source+account+platform composite。其他幂等/冲突/162+87合同不变，fresh RC3后再做一次完整QA。
- AR-034F RC3 / Independent Full QA：fresh RC=`release/ar034f-rc3-20260717@ad3d48e704f7f96b47157e1a37ce511f055a52f4`，direct parent=`07940e8...`，tree=`89f3d1ae...`，patch SHA=`23505db7...`。PM复核确认RC2->RC3产品差异仅为URL exclusive anchor、optional published_at和对应测试；named matcher与136+26 fixture通过，其他writer/reconcile/162+87/idempotency合同不变。固定QA线程已接收一次完整QA，并须以exact RC3做production GET-only分类；仅在planned=162、gap=26、conflict/duplicate/wrong-run/writes=0时可进入PM Production Authorization。
- AR-034F RC3 QA Failed / Data Contract Review：exact RC3 matcher 12/12与428 Python通过，但production GET-only仍为136 exact /0 legacy PUT /0 absent POST /26 conflict，0 write；21 AIHOT、2公众号、3对标视频全部BLOCK。optional published_at不是唯一根因，禁止继续自动弱化identity matcher。QA仅追加一次read-only字段级差异诊断，按候选数量、旧指纹占用、run及title/source/platform/account/time缺失或冲突分类；诊断后由PM先做产品/数据合同决策，再决定纯数据迁移、item-local隔离或停止当日恢复，不直接派新RC。
- AR-034G Canonical Owner Deduplication：字段诊断证明26/26均唯一命中同批次已有非空fingerprint owner，22条还与其他planned item共享owner；不存在空指纹、真实缺失、多候选或wrong-run。业务决策改为`162 raw planned -> 140 unique owners`：136 direct、22 shared aliases折叠、4 additional existing owners接管剩余aliases；不PUT/POST/覆盖/删除03。固定dev线程只实现owner投影、alias provenance、140条read-back与去重后候选输入，保留既有评分/主编逻辑；一个production-base窄RC、一次QA，不再修AR-034F matcher。
- AR-034G RC1 PM Review Failed / New-owner dedup rework：RC1=`0981500e...`正确关闭当前162->140 existing-owner恢复，但future writer只对已有Feishu owner去重；同批两个全新同URL planned item在empty snapshot下仍生成2个new owners并排队2次create。PM exact probe复现`unique=2/new=2`，会让重复问题在下一次运行重现，故未派QA、RC1不得发布。fresh RC2须先按URL/URL-empty composite对raw planned分组；无existing owner时确定性选一个new owner，其余aliases，writer每owner key最多create一次；当前162->140/0写结果保持不变。
- AR-034G RC2 / Independent Full QA：fresh RC=`release/ar034g-rc2-20260717@207060c1877afd3a96a27a85a4268de6c82043e9`，direct parent=`07940e8...`，tree=`2d73bffd...`，5 files，patch SHA=`1d919d72...`。PM同一反例重跑为2 raw ->1 owner/1 new/1 alias，37 focused通过；RC1->RC2只改raw owner-key grouping与测试，当前162->140 existing-owner/0写不变。固定QA线程已接收一次完整QA，须fresh GET-only验证production实际140 owners与candidate owner invariant；通过仅进入PM Production Authorization。
- AR-034G RC2 / Production Release Authorized：独立QA已通过L0、future-new dedup 12/12、full Python 417、receiver 32、Douyin 8+39、semantic 6+7及production GET-only。live结果为162 raw ->140 same-run owners，136 direct、26 aliases、22 shared、4 additional、0 new/0 write；140 owner read-back精确，候选11条均属于owner set。用户要求停止追加修改并直接发布；PM接受RC2进入production，不再派development或新RC。production线程只发布exact RC2、跑动态gate与现有正式恢复入口；任何真实运行失败如实停止，不以预防性假设扩展规则。
- AR-034G RC2 / Released：production已normal fast-forward/push至`207060c1877afd3a96a27a85a4268de6c82043e9`，tree=`2d73bffd...`，local=origin/main clean；dynamic release gate通过。released-code check-only复核同一run为162 raw ->140 owners、26 aliases、0 new/0 write、candidate=11，owner/read-back/source counts与QA一致。按release-only授权停止，未继续03/watermark/editorial/04/card，三automation保持PAUSED；证据根=`/private/tmp/ar034g_rc2_production_20260717_1230`。
- AR-034G / Same-day Business Recovery Authorized：用户纠正release-only范围，要求继续正常业务恢复并只在今天采集数据不存在时才重采。现有`run_20260717_093104`已验证162 raw、140 owners、11 candidates，因此本次明确跳过collection/provider refresh，从released RC2的owner-aware现有数据入口重跑候选->主编->04->个人Topic Card；成功后仅status恢复三automation，保留其余定义。不得再改代码或建RC，失败按真实业务错误停止并回传。
- AR-034G / Watermark Continuation Authorized：same-day恢复已完成140-owner/03零写闭环和11->6主编流程，但production因缺少独立watermark CLI拒绝调用已发布底层commit函数。PM判定这是执行入口形式问题，不是新业务风险；用户既有正常恢复授权已覆盖一次exact signed-receipt-bound watermark commit/read-back。禁止hotfix/新RC/直接改文件，固定production线程获准调用released正式函数一次，成功后继续04/card/status-only resume。
- AR-034G / Watermark After-commit Accepted：released commit函数已一次成功，canonical watermark schema2/run/attempt/revision/article watermark与DB exact，SHA=`a063fb89...`。随后health check因watermark已记录同一attempt而按代码预期返回`refresh_receipt_replayed`；该检查属于pre-commit anti-replay，不是after-commit失败。PM接受direct canonical read-back+DB parity，授权继续一次04、card和status-only resume，不改代码或重跑watermark。
- AR-034G / 04 Visible-field Correction：04已一次写入4条且run/count/duplicate均正确，但4/4`可沉淀资产`均为同一内部过程占位语，正式一致性门判too generic。PM判定这是用户可见内容质量问题而非安全扩scope；不改代码、不新建RC，固定production线程仅基于现有dossier/decision trace为4条生成各自具体资产，update exact record IDs、read-back/validator green后发卡并status-only resume。
...
- AR-034G / Same-run State Closure：Kimi 单字段兼容措辞修正后，canonical/latest CSV SHA=`8a91f24b...`，Feishu 04 official consistency 已 `4/4` 全绿；当前 `today_downstream_not_usable` 仅因 canonical `daily_pipeline_2026-07-17.json` 仍保留旧失败态（缺 `downstream_usable`，`recovered_ok/editorial_finalized/finalization_ok=false`），不是采集、03、04 或内容质量失败。固定 production 线程已获准只用 released helper 和 preserved same-run evidence 重算下游可用性并收口状态日志；必须保留 `full_collection_success=false/completed_with_failures`，只在 `downstream_usable=true` 且 04/read-back exact 时让 card guard 放行。禁止新 RC、改代码、重采、重复 04、绕守卫发卡或改 automation 非 status 字段。
- AR-034G / Card Source-title Normalization：canonical state closure 已成功，card check-only=`fresh/would_send=true`；实际 sender 在 API 前只因 fingerprint `38cf0a7c4bb24668` 的抖音来源把同一 caption 同时写入 `原始来源标题` 与 `原始发布文案` 而阻断。PM确认其余3条无重复，项目渲染合同已支持“平台未提供独立标题”。固定production线程获准只把该exact 04/local row的 `原始来源标题` 规范为该占位语，保留发布文案和所有选题字段；4/4 validator/card build green后发送一次personal card，再status-only恢复三automation。禁止代码/RC、门禁放宽、重采或其他内容修改。
- AR-034G Follow-up / Same-run Card Guard Observability：个人 Topic Card 已成功发送一次，message_id=`om_x100b6aa90ea9d480de2d14483d9b0e9`；但发送后 `run_topic_card_if_fresh --check-only` 仍显示 `would_send=true`，因为当前 guard 只拦 status-unknown，未把 succeeded ledger 显示为已发送。正式 sender 的 Feishu UUID 由 run+target+page deterministic 生成，transport 层具备同批幂等；本日禁止再次调用 sender。该项列为后续窄可观测性/应用幂等改进，不阻断新run的正常schedule，不在本次恢复中开新RC。
- AR-034G / Manual Projectless Entrypoint Repair：Completed。三automation均ACTIVE/projectless；用户已在三条prompt正文开头加入生产目录切换指令，read-back确认其位于所有相对`scripts/...`命令之前。无需改英文09:15协议，不手工补跑旧run；等待正常08:00/09:15/10:00 schedule观察。
- AR-035 / Script Package Watcher Codex CLI Path Repair：Independent QA Running。第二张 Topic Card 的点击、receiver 与队列均正常，但 production watcher 仍硬编码已不存在的 `/Applications/Codex.app/Contents/Resources/codex`，当前安装路径为 `/Applications/ChatGPT.app/Contents/Resources/codex`，导致生成器尚未启动、本地和飞书 06 都为 0。fresh RC=`3bfb11bc...`仅修 runner/installer 共用的 CLI 解析，PM scope/hash/dry-run证据门已通过；固定QA线程正在做一次独立验证。当前不改 production/runtime/LaunchAgent/Feishu，不新增重试、身份或安全架构。PM 误建的 scratch worktree 保持未提交、不得作为发布候选。
