...
- 测试表和生产表不能混用。任何会写入记录或修改状态的测试都必须使用测试表或 staging Base。
- 测试文档和生产文档不能混用。任何会创建飞书文档的测试都必须写入测试文件夹。
- 测试通知和生产通知不能混用。测试卡片、学习日结、失败 QA 演练默认发个人 open_id；不复用正式群。
- 测试环境必须显式加载 `AI_ACCOUNT_RADAR_ENV_FILE=.env.staging.local` 或 `AI_ACCOUNT_RADAR_ENV=staging`。显式 env 文件不会回退读取 `.env.local`。
- 暂时缺少测试资源时，只允许做生产只读检查，不允许写生产表或生产文件夹。
