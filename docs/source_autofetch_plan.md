...
`04 今日候选池` 的前台语义是“工作流实验命题卡候选池”，不是标题榜。采集源只负责提供素材；主编 Skill 会把素材转成短 `选题命题 / 我要做的实验 / 热点触发点 / 我的工作流痛点 / 旧流程痛点 / AI介入点 / 验证方式 / 可沉淀资产`。`可发布标题` 只有在 `title_permission=可发布标题` 时才生成。
