...
- 历史维护说明：`wewe-rss` 仅保留为已归档的本地维护工具，不是正常采集 runtime，也不能补当天内容。
- 这类方案通常需要单独部署、维护、登录态或服务配置，不能直接接入默认流程。
- 当前 P1 只允许显式拉取，不进入默认流程；正式默认化前需要连续稳定性、去重和字段完整性验证。
- 正常路径只允许公开发现 exact `mp.weixin.qq.com` URL 后直接解析全文；发现或全文失败时该 source 本轮为 0 行。
...
- 全文 provider：公开发现 exact URL 后使用当前正文解析器；归档 `wewe-rss` 不参与正常 runtime。
