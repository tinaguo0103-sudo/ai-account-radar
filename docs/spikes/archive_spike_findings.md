...
- 删除判断：旧候选验证脚本、公共 feed intake 和长报告可删除；结论已迁移到 source_autofetch_plan 和全文 provider 评估文档。
- 未保留内容：旧公共 feed intake 脚本、旧公共 feed 配置、旧候选验证脚本和长报告。
...
- 迁移到：
  ...
  - `docs/spikes/wechat_fulltext_provider_eval.md`
- 删除判断：可删除。main 只保留 wewe-rss fulltext provider adapter，不再保留公共 feed intake 或旧候选验证 POC。
- 未保留内容：旧公共 feed source watch 配置、旧公共 feed POC、旧公共 feed intake 脚本和配置。
...
- 后续如需继续做公众号自动发现，应新开小分支，并基于新的 `source_watch_probe` 实现；不得把实验依赖和外部仓库提交进 main。
