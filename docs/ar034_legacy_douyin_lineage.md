...
The public CLI owns the raw `ai_account_radar` production path beside the executing repository. Before resolving or reading evidence it opens that directory with `O_DIRECTORY | O_NOFOLLOW`, checks current-UID ownership, compares path and fd device/inode, and requires realpath to equal the raw absolute path. It does not infer trust from an evidence path and exposes no root override. Tests may inject an expected root only through the Python validator API. The root path/device/inode/UID are stored in the initial report and revalidated by locked pre-write validation.
