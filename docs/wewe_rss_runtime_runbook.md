...
The refresh mutual-exclusion lock is project-owned mutable coordination state at `output/state/wewe-refresh/refresh.lock` under the active production repository. The public adapter `--check-only` path must acquire and release this exact project-writable lock while reporting zero provider requests and zero secret reads. The former `~/.codex/.../providers/wewe-rss/health/refresh.lock` path is forbidden for scheduled locking. Immutable signed lease records, attempt lineage, receipts, the accepted watermark, provider SQLite data, attestation key, and runtime/container identity remain in the canonical owner-only `~/.codex/ai-account-radar-runtime/providers/wewe-rss` tree.
...
The 08:00 automation command and Prompt semantics do not change for this lock relocation because the public entrypoint and arguments are unchanged. Do not edit live automation for this change.
